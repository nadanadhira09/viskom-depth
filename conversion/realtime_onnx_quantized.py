"""
realtime_onnx_quantized.py
==========================
Menggunakan ONNX Quantized INT8 model (~3x lebih cepat dari PyTorch!)
"""
...
class ONNXQuantizedDepth:
    def __init__(self, model_path='models/onnx/depth_anything_v2_vits_quantized_int8.onnx'):
        """Load quantized ONNX model (INT8)"""
        print(f"[INFO] Loading Quantized ONNX: {model_path}")
        ...
        print(f"[OK] Quantized model loaded")
...
def run_realtime(skip=1):
    ...
    print("="*70)
    print("REAL-TIME ONNX QUANTIZED INT8 (~3x Faster!)")
    print("="*70)
    
    # Check model exists
    model_path = Path('models/onnx/depth_anything_v2_vits_quantized_int8.onnx')
    ...
    # Load models
    print(f"\n[INFO] Loading models...")
    depth_model = ONNXQuantizedDepth(str(model_path))
    ...
    try:
        while True:
            ...
            # Normalize depth (sama dengan official repo)
            ...
            # Add overlay
            cv2.putText(combined, f"FPS: {avg_fps:.1f} (ONNX Quantized INT8)", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
            cv2.putText(combined, f"Frame: {frame_count:4d} | Proc: {proc_count:4d}", (10, 70),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            cv2.putText(combined, f"Time: {elapsed*1000:.1f}ms | Mode: {'YOLO+' if use_yolo else ''}Depth", 
                       (10, h-20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 0), 1)
            
            cv2.imshow("ONNX Quantized INT8 (Fastest!) - Left: RGB | Right: Depth", combined)
